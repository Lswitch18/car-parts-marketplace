...
resource "aws_instance" "mac_builder" {
  ...
  # Configuração do Mac: SSH, VNC e Toolchains
  user_data = <<-EOF
    #!/bin/bash
    # 1. Senha ec2-user
    ...
    # 3. Habilita VNC (Apple Remote Desktop / Compartilhamento de Tela)
    /System/Library/CoreServices/RemoteManagement/ARDAgent.app/Contents/Resources/kickstart \
      -activate -configure -access -on -restart -agent -privs -all || true

    # 4. Toolchain base (best-effort — não bloqueia boot se falhar)

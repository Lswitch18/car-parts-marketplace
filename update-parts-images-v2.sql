...
-- GT-R RB26DETT
UPDATE parts
SET images = ARRAY['/parts-images/engine-rb26dett.png'],
    updated_at = NOW()
WHERE title ILIKE '%RB26DETT%' OR (title ILIKE '%GT-R%' AND title ILIKE '%Motor%');

-- Supra 2JZ-GTE
UPDATE parts
SET images = ARRAY['/parts-images/engine-2jzgte.png'],
    updated_at = NOW()
...
WHERE (title ILIKE '%Turbo%' OR title ILIKE '%GTX%' OR title ILIKE '%GT3582%' OR title ILIKE '%TD06%')
  AND images NOT LIKE '%parts-images%';
...
UPDATE parts
SET images = ARRAY['/parts-images/wheels-work-meister.png'],
    updated_at = NOW()
...
UPDATE parts
SET images = ARRAY['/parts-images/exhaust-hks-hipower.png'],
    updated_at = NOW()
...
UPDATE parts
SET images = ARRAY['/parts-images/bodykit-veilside-rx7.png'],
    updated_at = NOW()

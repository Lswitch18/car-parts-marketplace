  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div><h2 className="text-2xl font-bold">Ocorrências</h2><p className="text-sm text-gray-400 mt-1">{list.length} registros</p></div>
        <button onClick={openCreate} className="h-10 px-4 bg-blue-500 hover:bg-blue-600 text-white rounded-lg font-medium flex items-center gap-2 text-sm"><Plus size={16} /> Nova Ocorrência</button>
      </div>
      ...
      <div className="bg-[#111827] rounded-xl border border-white/5 overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody>
              {list.length === 0 ? <tr><td colSpan={6} className="text-center py-12 text-gray-500 text-sm">Nenhuma ocorrência</td></tr>
              : list.map((row: any) => (
                <tr key={row.id} className="border-b border-white/5 hover:bg-white/[0.02] transition-colors">
                  ...
                  <td className="p-4"><span className={`text-xs px-2 py-0.5 rounded-full ${
                    ...
                    row.status === 'resolvido' ? 'text-green-400 bg-green-400/15' : 'text-gray-400 bg-gray-400/15'
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={closeModal}>
          <div className="bg-[#1F2937] rounded-xl p-6 w-full max-w-lg mx-4 border border-white/10" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-4">
              ...
              <div><label className="text-sm text-gray-400 mb-1 block">Descrição</label>
                <textarea value={form.descricao} onChange={e => setForm({ ...form, descricao: e.target.value })} rows={3} className="w-full bg-[#111827] border border-white/10 rounded-lg px-4 py-2.5 text-sm text-white outline-none focus:border-blue-500 resize-none" />
              </div>
            </div>

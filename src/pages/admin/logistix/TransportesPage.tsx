  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div><h2 className="text-2xl font-bold">Transportes</h2><p className="text-sm text-gray-400 mt-1">{list.length} veículos cadastrados</p></div>
        <button onClick={openCreate} className="h-10 px-4 bg-blue-500 hover:bg-blue-600 text-white rounded-lg font-medium flex items-center gap-2 text-sm"><Plus size={16} /> Novo Veículo</button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {list.length === 0 ? (
          <div className="col-span-full text-center py-12 text-gray-500">Nenhum veículo encontrado</div>
        ) : list.map((row: any) => (
          <div key={row.id} className="bg-[#111827] rounded-xl border border-white/5 p-5 hover:border-blue-500/30 transition-all group">
            ...
            <p className="text-sm text-gray-400 font-mono">{row.placa || 'Sem placa'}</p>
            <div className="flex items-center gap-2 mt-3 text-xs">
              <span className="px-2 py-0.5 rounded-full bg-gray-500/20 text-gray-300">{row.tipo || 'caminhao'}</span>
              <span className="text-gray-400">{row.capacidade_kg}kg</span>
              <span className={`px-2 py-0.5 rounded-full ${row.ativo ? 'text-green-400 bg-green-400/15' : 'text-gray-400 bg-gray-400/15'}`}>{row.ativo ? 'Ativo' : 'Inativo'}</span>
            </div>
            <p className="text-xs text-gray-500 mt-2">Armazém: {row.armazem?.nome || row.armazem_id?.substring(0, 8) || '-'}</p>
          </div>
        ))}
      </div>
          ...
          <div className="bg-[#1F2937] rounded-xl p-6 w-full max-w-lg mx-4 border border-white/10" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div><label className="text-sm text-gray-400 mb-1 block">Placa</label><input value={form.placa} onChange={e => setForm({ ...form, placa: e.target.value })} className="w-full bg-[#111827] border border-white/10 rounded-lg px-4 py-2.5 text-sm text-white outline-none focus:border-blue-500" /></div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div><label className="text-sm text-gray-400 mb-1 block">Tipo</label>
                  <select value={form.tipo} onChange={e => setForm({ ...form, tipo: e.target.value })} className="w-full bg-[#111827] border border-white/10 rounded-lg px-4 py-2.5 text-sm text-white outline-none focus:border-blue-500">
                ...
                <div><label className="text-sm text-gray-400 mb-1 block">Capacidade (kg)</label><input type="number" value={form.capacidade_kg} onChange={e => setForm({ ...form, capacidade_kg: Number(e.target.value) })} className="w-full bg-[#111827] border border-white/10 rounded-lg px-4 py-2.5 text-sm text-white outline-none focus:border-blue-500" /></div>
              </div>

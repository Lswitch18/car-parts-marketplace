  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div><h2 className="text-2xl font-bold">Transferências</h2><p className="text-sm text-text-secondary mt-1">{list.length} transferências</p></div>
        <button onClick={() => setShowModal(true)} className="h-10 px-4 bg-primary hover:bg-primary-dark text-white rounded-lg font-medium flex items-center gap-2 text-sm"><Plus size={16} /> Nova Transferência</button>
      </div>
      ...
      <div className="bg-[#111827] rounded-xl border border-border overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody>
              {list.length === 0 ? <tr><td colSpan={6} className="text-center py-12 text-text-muted text-sm">Nenhuma transferência</td></tr>
              : list.map((row: any, idx: number) => (
                <tr key={row.id || idx} className="border-b border-border hover:bg-surface/[0.02] transition-colors">
                  <td className="p-4 text-sm text-white">{row.produto}</td>
                  <td className="p-4 text-sm text-gray-300">{row.quantidade}</td>
                  <td className="p-4 text-sm text-text-secondary">{row.origem?.nome || row.armazem_origem_id?.substring(0, 8)}</td>
                  <td className="p-4 text-sm text-text-secondary">{row.destino?.nome || row.armazem_destino_id?.substring(0, 8)}</td>
          ...
          <div className="bg-[#1F2937] rounded-xl p-6 w-full max-w-lg mx-4 border border-border" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div><label className="text-sm text-text-secondary mb-1 block">Origem</label>
                ...
                <div><label className="text-sm text-text-secondary mb-1 block">Destino</label>
                  <select value={form.armazem_destino_id} onChange={e => setForm({ ...form, armazem_destino_id: e.target.value })} className="w-full bg-[#111827] border border-border rounded-lg px-4 py-2.5 text-sm text-white outline-none focus:border-blue-500">
                    <option value="">Selecione...</option>
                    {(armazens || []).map((a: any) => <option key={a.id} value={a.id}>{a.nome}</option>)}
                  </select>
                </div>
            ...
            <div className="flex items-center gap-3 mt-6 justify-end">
              ...
              <button onClick={() => createMutation.mutate()} disabled={createMutation.isPending} className="h-10 px-4 bg-primary hover:bg-primary-dark text-white rounded-lg text-sm font-medium disabled:opacity-50">Criar Transferência</button>
            </div>
          </div>
        </div>
      )}

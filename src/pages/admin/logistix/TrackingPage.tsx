  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-sm text-gray-400 mt-1">Busque pelo código da etiqueta</p>
      </div>

      <form onSubmit={e => { e.preventDefault(); setCodigo(search.trim()); }}
        className="flex items-center gap-2 bg-[#111827] rounded-lg h-11 px-3 border border-white/5">
        ...
        <button type="submit" className="h-8 px-4 bg-blue-500 rounded-lg text-xs font-semibold whitespace-nowrap">
          Buscar
        </button>
      </form>
      ...
      {isLoaded && !pedido && (
        <div className="text-center py-12 bg-[#111827] rounded-xl">
          ...
          <p className="text-gray-400 font-medium">Pedido não encontrado</p>
          <p className="text-xs text-gray-500 mt-1">Verifique o código digitado</p>
        </div>
      )}
      ...
      {pedido && (
        <div className="bg-[#111827] rounded-xl border border-white/5 overflow-hidden">
          ...
          <div className="p-5 border-b border-white/5">
            ...
            {pedido.peso_kg && (
              <p className="text-xs text-gray-500 mt-1">{pedido.peso_kg}kg · ¥{(pedido.valor || 0).toLocaleString()}</p>
            )}
            <button onClick={() => setShowLabel(true)}
              className="mt-3 h-9 px-4 bg-blue-500/10 hover:bg-blue-500/20 border border-blue-500/30 rounded-lg text-xs font-medium text-blue-400 flex items-center gap-2">
                  ...
                  <div key={ev.id} className="flex gap-3 pb-4 relative">
                    <div className="flex flex-col items-center">
                      <div className={`w-3 h-3 rounded-full border-2 z-10 ${
                        idx === 0 ? 'bg-blue-500 border-blue-400' : 'bg-[#111827] border-gray-600'
                      }`} />
                    ...
                    <div className="flex-1 -mt-0.5">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] text-gray-500">{new Date(ev.created_at).toLocaleString('pt-BR')}</span>
                      </div>

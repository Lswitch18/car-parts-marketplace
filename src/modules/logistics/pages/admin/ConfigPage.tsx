  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-sm text-text-secondary mt-1">Gerenciar configurações do sistema</p>
      </div>

      <div className="bg-[#111827] rounded-xl border border-border p-6">
        <h3 className="text-base font-medium mb-4 flex items-center gap-2">
          <Settings size={16} className="text-text-secondary" /> Configurações do Sistema
        </h3>
        <div className="space-y-4">
          {entries.length === 0 ? (
            <p className="text-sm text-text-muted text-center py-4">Nenhuma configuração encontrada</p>
          ) : entries.map(([chave, valor]) => (
            <div key={chave} className="flex items-center justify-between py-3 border-b border-border last:border-b-0">
              <div>
                <p className="text-sm font-medium text-white">{chave}</p>
                <p className="text-xs text-text-secondary mt-0.5 font-mono">{String(valor)}</p>
              </div>
              <input
                value={String(valor)}
                onChange={e => {
                ...
                className="bg-[#0B1220] border border-border rounded-lg px-3 py-1.5 text-sm text-white outline-none focus:border-blue-500 w-48 text-right"
              />
            </div>
          ))}
        </div>
      ...
      <div className="bg-[#111827] rounded-xl border border-border p-6">
        ...
        <div className="flex items-center gap-3">
          <input placeholder="Chave" value={newKey} onChange={e => setNewKey(e.target.value)}
            className="flex-1 bg-[#0B1220] border border-border rounded-lg px-4 py-2.5 text-sm text-white outline-none focus:border-blue-500" />

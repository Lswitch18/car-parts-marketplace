  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div><h2 className="text-2xl font-bold">Clientes</h2><p className="text-sm text-text-secondary mt-1">{total} clientes</p></div>
        <button onClick={openCreate} className="h-10 px-4 bg-primary hover:bg-primary-dark text-white rounded-lg font-medium flex items-center gap-2 text-sm"><Plus size={16} /> Novo Cliente</button>
      </div>
      ...
      <div className="bg-[#111827] rounded-xl border border-border overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody>
              {rows.length === 0 ? <tr><td colSpan={7} className="text-center py-12 text-text-muted text-sm">Nenhum cliente encontrado</td></tr>
              : rows.map((row: any) => (
                <tr key={row.id} className="border-b border-border hover:bg-surface/[0.02] transition-colors">
                  ...
                  <td className="p-4 text-sm text-text-secondary font-mono">{row.cnpj || '-'}</td>
                  <td className="p-4"><span className={`text-xs px-2 py-0.5 rounded-full ${row.ativo ? 'text-green-400 bg-green-400/15' : 'text-text-secondary bg-gray-400/15'}`}>{row.ativo ? 'Sim' : 'Não'}</span></td>
                  <td className="p-4"><div className="flex items-center gap-2">
                    ...
                    <button onClick={() => deleteMutation.mutate(row.id)} className="p-1.5 hover:bg-surface/5 rounded-lg text-text-secondary hover:text-red-400"><Trash2 size={14} /></button>
                  </div></td>
                </tr>
              ))}
            </tbody>
          ...
          <div className="bg-[#1F2937] rounded-xl p-6 w-full max-w-lg mx-4 border border-border" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div><label className="text-sm text-text-secondary mb-1 block">Email</label><input type="email" value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} className="w-full bg-[#111827] border border-border rounded-lg px-4 py-2.5 text-sm text-white outline-none focus:border-blue-500" /></div>
              ...
              <div><label className="text-sm text-text-secondary mb-1 block">CNPJ</label><input value={form.cnpj} onChange={e => setForm({ ...form, cnpj: e.target.value })} className="w-full bg-[#111827] border border-border rounded-lg px-4 py-2.5 text-sm text-white outline-none focus:border-blue-500" /></div>

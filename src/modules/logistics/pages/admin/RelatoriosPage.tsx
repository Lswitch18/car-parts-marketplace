    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-text-secondary mt-1">Métricas e desempenho logístico</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button 
            ...
            className="h-10 px-4 bg-primary hover:bg-primary-dark text-white rounded-lg text-sm font-medium flex items-center gap-2"
          >
      ...
      <div className="grid grid-cols-5 gap-4">
        {stats.map((s, i) => (
          <div key={i} className="bg-[#111827] rounded-xl p-5 border border-border">
            <div className="flex items-center gap-2 mb-3">
              <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ background: `${s.color}22` }}>
                <s.icon size={16} style={{ color: s.color }} />
              </div>
            </div>
            <p className="text-[13px] text-text-secondary mb-1">{s.label}</p>
      ...
      <div className="grid grid-cols-2 gap-5">
        <div className="bg-[#111827] rounded-xl p-5 border border-border">
          <h3 className="text-base font-medium mb-4 flex items-center gap-2">
            <FileText size={16} className="text-text-secondary" /> Pedidos Recentes
          </h3>
          <div className="space-y-3">
            {(!recentOrders || recentOrders.length === 0) ? (
              <p className="text-sm text-text-muted text-center py-4">Nenhum pedido recente</p>
            ) : recentOrders.slice(0, 8).map((row: any, i: number) => (
              <div key={i} className="flex items-center justify-between text-sm">
                <span className="font-mono text-blue-400">{row.codigo}</span>
                <span className="text-text-secondary">{row.cliente}</span>
                <span className={`px-2 py-0.5 rounded text-[11px] ${
                  ...
                  'text-text-secondary bg-gray-400/15'
        ...
        <div className="bg-[#111827] rounded-xl p-5 border border-border">
          <h3 className="text-base font-medium mb-4 flex items-center gap-2">
            <BarChart3 size={16} className="text-text-secondary" /> Indicadores
          </h3>
          <div className="space-y-4">
            ...
            <div>
              <div className="flex justify-between text-sm mb-1">
                <span className="text-text-secondary">Em Trânsito</span>
                <span className="text-blue-400 font-medium">{((kpis?.emTransito || 0) / Math.max(kpis?.total || 1, 1) * 100).toFixed(1)}%</span>
              </div>
              <div className="h-2 bg-surface/5 rounded-full overflow-hidden">
                <div className="h-full rounded-full bg-primary" style={{ width: `${Math.min((kpis?.emTransito || 0) / Math.max(kpis?.total || 1, 1) * 100, 100)}%` }} />
              </div>
            </div>
            <div>
              <div className="flex justify-between text-sm mb-1">
                <span className="text-text-secondary">Atrasos</span>
            ...
            <div>
              <div className="flex justify-between text-sm mb-1">
                <span className="text-text-secondary">Cancelamentos</span>
